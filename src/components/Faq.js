...
const Faq = () => {
  const QnA = [
    ...
    },
    {
      ...
      ans: "Mat nunc sed blandit libero volutpat3. Tortor at risus viverra adipiscing at in tellus3.",
    },
    {
    ...
    <div className='faqs'>
      <div className='faqs-content'>
        {QnA.map((question, index) => (
          <div key={index}>
            <h4 onClick={() => toggleAnswer(index)} className='inLine'>
              {question.question}
            </h4>
            <button className='inLine' onClick={() => toggleAnswer(index)}>
              <BsFillPlusCircleFill />
            </button>
            <div>{show[index] && <p>{question.ans}</p>}</div>
          </div>
        ))}
      </div>
